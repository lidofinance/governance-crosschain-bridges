...
makeSuite('Crosschain bridge tests', setupTestEnvironment, (testEnv: TestEnv) => {
  ...
  before(async () => {
    ...
    /**
     * Linea -- Create Proposal Actions 19
     * Update Ethereum Governance Executor in the Linea Governance contract
     */
    const proposal19Actions = await createLineaBridgeTest(aaveWhale2.address, testEnv);
    ...
    await expectProposalState(aaveGovContract, proposals[18].id, proposalStates.QUEUED);
  ...
  describe('LineaBridgeExecutor Authorization', async function () {
    it('Unauthorized Transaction - Call Bridge Receiver From Non-EthereumGovernanceExecutor Address', async () => {
      const { lineaBridgeExecutor } = testEnv;
      const { targets, values, signatures, calldatas, withDelegatecalls } =
        testEnv.proposalActions[0];
      await expect(
        lineaBridgeExecutor.queue(targets, values, signatures, calldatas, withDelegatecalls)
      ).to.be.revertedWith(ExecutorErrors.UnauthorizedEthereumExecutor);
    });
    it('Unauthorized Update Ethereum Governance Executor - revert', async () => {
      const { lineaBridgeExecutor, aaveWhale1 } = testEnv;
      await expect(
        lineaBridgeExecutor
          .connect(aaveWhale1.signer)
          .updateEthereumGovernanceExecutor(aaveWhale1.address)
  ...
  describe('Queue - LineaBridgeExecutor through Ethereum Aave Governance', async function () {
    it('Execute Proposal 19 - successfully queue Linea transaction - duplicate polygon actions', async () => {
      const { ethers } = DRE;
      const { aaveGovContract, shortExecutor, lineaBridgeExecutor, lineaMessageService } = testEnv;

      const { targets, values, signatures, calldatas, withDelegatecalls } =
        testEnv.proposalActions[18];

      // Mock sender
      await lineaMessageService.setSender(shortExecutor.address);
      ...
      await expect(aaveGovContract.execute(proposals[18].id, overrides))
        .to.emit(lineaBridgeExecutor, 'ActionsSetQueued')
        .withArgs(
          0,
          targets,
          values,
  ...
  describe('Execute Action Sets - Aave Linea Governance', async function () {
    it('Execute Action Set 0 - update ethereum governance executor', async () => {
      const { lineaBridgeExecutor, lineaMessageService, shortExecutor, aaveWhale2, aaveGovOwner } =
        testEnv;

      // Mock sender
      await lineaMessageService.setSender(shortExecutor.address);

      await expect(lineaBridgeExecutor.execute(0))
        .to.emit(lineaBridgeExecutor, 'ActionsSetExecuted')
        .withArgs(0, aaveGovOwner.address, ['0x'])
        .to.emit(lineaBridgeExecutor, 'EthereumGovernanceExecutorUpdate')
        .withArgs(
          DRE.ethers.utils.getAddress(shortExecutor.address),
          DRE.ethers.utils.getAddress(aaveWhale2.address)
        );
  ...
  describe('LineaBridgeExecutor Getters - EthereumGovernanceExecutor', async function () {
    it('Get EthereumGovernanceExecutor', async () => {
      const { lineaBridgeExecutor, aaveWhale2 } = testEnv;
      expect(await lineaBridgeExecutor.getEthereumGovernanceExecutor()).to.be.equal(
        DRE.ethers.utils.getAddress(aaveWhale2.address)
      );
    });
  });
